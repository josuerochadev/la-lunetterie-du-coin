import { useRef } from 'react';
import { m, useScroll, useTransform, useSpring } from 'framer-motion';

import { SimpleAnimation } from '@/components/motion/SimpleAnimation';
import LinkCTA from '@/components/common/LinkCTA';
import { usePrefersReducedMotion } from '@/hooks/usePrefersReducedMotion';

const SPRING_CONFIG = { stiffness: 80, damping: 30, mass: 0.5 };
const ZOOM_SPRING = { stiffness: 60, damping: 30, mass: 0.5 };

// ---------------------------------------------------------------------------
// Desktop animated — "VENEZ" zoom + staggered words + CTA
//
//  250vh container. Mirrors HomeContact pattern.
//
//  0.00 – 0.02  Yellow bg instant reveal
//  0.00 – 0.20  "VENEZ" zooms scale 12 → 1
//  0.14 – 0.22  "NOUS" slides up
//  0.19 – 0.27  "VOIR" slides up
//  0.26 – 0.34  CTA links fade in
//  0.34 – 1.00  Hold — footer scrolls over
// ---------------------------------------------------------------------------

function CTADesktop() {
  const sectionRef = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ['start start', 'end end'],
  });

  // Yellow bg — instant reveal
  const yellowBg = useTransform(scrollYProgress, [0.0, 0.02], [0, 1]);

  // "VENEZ" zoom
  const venezScaleRaw = useTransform(scrollYProgress, [0.0, 0.2], [12, 1]);
  const venezScale = useSpring(venezScaleRaw, ZOOM_SPRING);

  // "NOUS"
  const nousOpacity = useTransform(scrollYProgress, [0.14, 0.22], [0, 1]);
  const nousYRaw = useTransform(scrollYProgress, [0.14, 0.22], [40, 0]);
  const nousY = useSpring(nousYRaw, SPRING_CONFIG);

  // "VOIR"
  const voirOpacity = useTransform(scrollYProgress, [0.19, 0.27], [0, 1]);
  const voirYRaw = useTransform(scrollYProgress, [0.19, 0.27], [40, 0]);
  const voirY = useSpring(voirYRaw, SPRING_CONFIG);

  // CTA links
  const ctaOpacity = useTransform(scrollYProgress, [0.26, 0.34], [0, 1]);
  const ctaYRaw = useTransform(scrollYProgress, [0.26, 0.34], [30, 0]);
  const ctaY = useSpring(ctaYRaw, SPRING_CONFIG);

  return (
    <div ref={sectionRef} className="hidden h-[250vh] lg:block">
      <div className="sticky top-0 h-screen overflow-hidden bg-background">
        {/* Yellow bg — instant reveal */}
        <m.div
          className="absolute inset-0 bg-accent"
          style={{ opacity: yellowBg }}
          aria-hidden="true"
        />

        {/* Content */}
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-10 px-8">
          <h2
            className="text-heading text-center text-black"
            style={{ fontSize: 'clamp(3.5rem, 12vw, 14rem)', lineHeight: '0.95' }}
          >
            <m.span className="block" style={{ opacity: nousOpacity, y: nousY }}>
              VENEZ
            </m.span>
            <m.span className="block" style={{ opacity: voirOpacity, y: voirY }}>
              NOUS
            </m.span>
            <m.span className="block origin-center" style={{ scale: venezScale }}>
              VOIR
            </m.span>
          </h2>

          <m.div
            className="flex flex-col items-center gap-6 sm:flex-row"
            style={{ opacity: ctaOpacity, y: ctaY }}
          >
            <LinkCTA to="/services" theme="accent">
              Découvrir nos services
            </LinkCTA>
            <LinkCTA to="/contact" theme="accent">
              Nous contacter
            </LinkCTA>
          </m.div>
        </div>
      </div>
    </div>
  );
}

// ---------------------------------------------------------------------------
// Main component
// ---------------------------------------------------------------------------

export default function AboutCTA() {
  const prefersReducedMotion = usePrefersReducedMotion();

  return (
    <section className="relative w-full bg-accent" data-navbar-theme="dark">
      {/* Desktop animated */}
      {!prefersReducedMotion && <CTADesktop />}

      {/* Mobile / reduced-motion fallback */}
      <div className={prefersReducedMotion ? '' : 'lg:hidden'}>
        <div className="mx-auto max-w-container px-container-x py-section">
          <div className="mx-auto max-w-4xl text-center">
            <SimpleAnimation type="slide-up" delay={0}>
              <h2
                className="text-heading mb-10 text-black"
                style={{ fontSize: 'clamp(3rem, 10vw, 5rem)', lineHeight: '0.95' }}
              >
                VENEZ
                <br />
                NOUS VOIR
              </h2>
            </SimpleAnimation>

            <SimpleAnimation type="fade" delay={100}>
              <div className="flex flex-col items-center justify-center gap-6 sm:flex-row">
                <LinkCTA to="/services" theme="accent">
                  Découvrir nos services
                </LinkCTA>
                <LinkCTA to="/contact" theme="accent">
                  Nous contacter
                </LinkCTA>
              </div>
            </SimpleAnimation>
          </div>
        </div>
      </div>
    </section>
  );
}
