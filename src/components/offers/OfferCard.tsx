...
export default function OfferCard({ offer, isOpen, onToggle }: OfferCardProps) {
  ...
  return (
    <button
      type="button"
      className="simple-hover-scale focus-style group relative h-card w-full cursor-pointer overflow-hidden rounded-card shadow-card transition-all duration-300 hover:shadow-lg"
      style={{
    ...
    >
      ...
      >
            ...
            >
              {isOpen ? (
                <div className="space-y-xs">
                  <p className="text-body-sm font-bold uppercase tracking-wide text-accent">
                    Détails
                  </p>
                  <p>{offer.details}</p>
                </div>
                ...
                <div>
                  <p className="mb-1 text-body-sm font-bold uppercase tracking-wide text-orange">
                    Résumé
                  </p>
                  <p>{offer.summary}</p>
                </div>
        ...
        <div className="mt-auto flex items-center justify-center gap-2 transition-colors duration-200 group-hover:text-accent">
          <span className="text-body-sm font-medium">{isOpen ? 'Réduire' : 'En savoir plus'}</span>
          <m.div
            animate={{
              rotate: isOpen ? 180 : 0,
              scale: isOpen ? 0.9 : 1,
